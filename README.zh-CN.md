...
| 祈祷没有提示注入 | 不可信知识文本经注入标记检测后**强制隔离**，永不进入 Agent 上下文 |
...
3. RAG 按可信等级检索知识证据；类提示注入记录保持可见供审查，但永不进入 Agent 上下文。
...
- 不可信知识文本在进入 Agent 之前经提示注入标记检测强制隔离；Agent 的证据 ID 由服务端对照可信子集校验。
